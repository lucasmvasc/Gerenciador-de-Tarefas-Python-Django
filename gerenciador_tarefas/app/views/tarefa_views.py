...
# Responsável pelo gerenciamento da aplicação - requisições (renderizar páginas HTML, gerenciar SQL...)
...
def cadastrar_tarefa(request):
    ...
    if request.method == "POST":
        ...
        if form_tarefa.is_valid():
            ...
            # Pegando dados do form, criando um obj Tarefa com esses dados e enviando para o tarefa_service
...
def editar_tarefa(request, id):
    # Pegar os dados antigos do form
    tarefa_bd = tarefa_service.listar_tarefas_id(id)
    ...
    if form_tarefa.is_valid():
        ...
        tarefa_nova = Tarefa(titulo=titulo, descricao=descricao, data_expiracao=data_expiracao,
                             prioridade=prioridade, usuario=request.user)
        # Momento em que as informações são substituidas
